...
public class ShowCreateStreamCommand extends ShowCommand {
    ...
    public <R, C> R accept(PlanVisitor<R, C> visitor, C context) {
        return visitor.visitShowCreateStreamCommand(this, context);
    }
    ...
    public ShowResultSet doRun(ConnectContext ctx, StmtExecutor executor) throws Exception {
        ...
        stream.readLock();
        try {
            List<String> createStreamStmt = Lists.newArrayList();
            ctx.getEnv().getDdlStmt(null, null, stream, createStreamStmt, null, null, false, true,
                    false, -1L, false, false);

            if (!createStreamStmt.isEmpty()) {
                rows.add(Lists.newArrayList(stream.getName(), createStreamStmt.get(0)));
            }
        } finally {
            stream.readUnlock();
        }
